...
async function handleMessage(message, phone, storeName, apiKey, options = {}) {
  ...
  if (state.current === 'COLLECT_ADDRESS' && state.address_confirming) {
    ...
    // "rider aajata hai" / "rider aajateen hai" / "address thek hai" = acceptance
    const isRiderAcceptance = /\b(rider|courier)\s*.*(aa\s*jat[aei]|ajat[aei]|aajat[aei]|aa\s*jaye?|ajaye?|pohch|pahoch|deliver)\b/i.test(l);
    if (isRiderAcceptance && /\b(thek|theek|thik|sahi|bilkul)\b/i.test(l)) {
      state.address_confirming = false;
      const addrStr = buildAddressString(state.collected.address_parts, state.collected.city);
      state.collected.address = state.collected.city ? addrStr + ', ' + state.collected.city : addrStr;
      const dp = state.collected.delivery_phone;
  ...
  try {
    ...
    if (extracted.name && (state.current === 'COLLECT_NAME' || state.current === 'PRODUCT_SELECTION' || (hasExplicitName && !isFamilyName && !nameAlreadyLocked))) {
      ...
      // Strip trailing "hai/he/h" (= "is" in Urdu) — "X Hai." → "X", "Ahmed Hai" → "Ahmed"
      name = name.replace(/\s+(hai|he|h)[.!۔]?\s*$/i, '').trim();
      ...
      const isGreetingName = /^(assalam|assalamu?\s*al[ae]i?ku?m|aslam\s*[ou]\s*a?l?[ae]i?ku?m|wa\s*al[ae]i?ku?m|salam|aoa|hello|hi|hey|g\s+bhej|ji\s+bhej)\b/i.test(nameL);
