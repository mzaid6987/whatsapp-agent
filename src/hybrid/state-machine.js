...
function handleTemplateState(message, state, storeName, preIntent) {
  ...
  const flexYes = /\b(ha+n|hm+|ji+|jee|g|yes+|yess+|sure|shi|sai|sahi|bilkul|confir\w*|ik|o?k+a*y+|o?ki+|ok\s*ok|ok[zgky]?|done|theek|thik|thk|tik|zaroor|kr\s*do|kardo|krdo|kar\s*do|bhej\s*d[oaeiy]+|bhejd[oaeiy]+|bhij\s*d[oaeiy]+|bhijd[oaeiy]+|bhaj\s*d[oaeiy]+|bhajd[oaeiy]+|bhwj\s*d[oaeiy]+|bhwjd[oaeiy]+|bhjdo|bhjd[oaeiy]+|bhejwa\s*d[oaeiy]+|bhijwa\s*d[oaeiy]+|bhijwad[oaeiy]+|bhejwad[oaeiy]+|mangwa\s*d[oaeiy]+|mangwad[oaeiy]+|mngwa\s*d[oaeiy]+|mngwad[oaeiy]+|laga\s*d[oaeiy]+|lagad[oaeiy]+|lgad[oaeiy]+|lga\s*d[oaeiy]+|bhej\s*dein|bhejo|mangao|manga\s*lo|book\s*kr|place\s*order)\b/i.test(l) || URDU_YES_WORD_RE.test(message);
