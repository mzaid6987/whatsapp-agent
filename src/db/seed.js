...
function seedProducts(db) {
  ...
  const PRODUCTS = [
    ...
    { id:7, name:'2-in-1 Facial Hair Remover', short:'Facial Hair Remover', price:1399, kw:'facial,eyebrow,face hair,hair remover,brow,face remover', f1:'Chehre ke baal aur eyebrows dono ke liye', f2:'painless hai aur rechargeable', upsell:'[2]' },
...
function syncProductPrices(db) {
  const PRICES = { 1: 1399, 2: 1700, 3: 1499, 4: 599, 5: 899, 6: 2099, 7: 1399, 8: 1999, 9: 999, 10: 1150 };
  const update = db.prepare('UPDATE products SET price = ? WHERE id = ? AND price != ?');
  let changed = 0;
  for (const [id, price] of Object.entries(PRICES)) {
    const r = update.run(price, Number(id), price);
    if (r.changes > 0) changed++;
  }
  if (changed > 0) console.log(`[DB] Synced ${changed} product prices`);
}
