...
async function main() {
  ...
  function api(path) {
    return new Promise((resolve, reject) => {
      ...
      https.get(url, { headers: { Cookie: sessionCookie }, rejectUnauthorized: false }, res => {
        let d = '';
        res.on('data', c => d += c);
        res.on('end', () => { try { resolve(JSON.parse(d)); } catch(e) { reject(new Error(`Parse error on ${path}: ${d.substring(0,100)}`)); } });
      }).on('error', reject);
    });
  }

  const convs = await api('/api/conversations');
  const meaningful = convs.filter(c => c.message_count >= 3).sort((a,b) => a.id - b.id);
  ...
  console.error(`Total remaining: ${remaining.length}`);
  
  // Count how many have no bodies at all  
  let emptyCount = 0;
  let batch = [];
  let emptyIds = [];
  ...
  for (const conv of remaining) {
    ...
    if (!hasContent) {
      emptyCount++;
      emptyIds.push(conv.id);
      continue;
    }
    batch.push({ conv, msgs });
  }
  
  console.error(`Empty chats (no message bodies): ${emptyCount} — IDs: ${emptyIds.join(', ')}`);
  console.error(`Batch 20 IDs: ${batch.map(b => b.conv.id).join(', ')}`);
  
  let output = '';
  for (const { conv, msgs } of batch) {
    output += `\n=== CHAT #${conv.id} === Phone: ${conv.customer_phone || '?'} | State: ${conv.state || '?'} | Msgs: ${conv.message_count}\n`;
    for (const m of msgs) {
      const who = m.direction === 'incoming' ? 'USER' : 'BOT';
      const txt = (m.body || '').substring(0, 500);
      if (txt.trim()) output += `${who}: ${txt}\n`;
    }
  }
  
  fs.writeFileSync('tmp_batch20_chats.txt', output, 'utf8');
  console.error('Written to tmp_batch20_chats.txt');
}
