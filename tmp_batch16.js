...
const loginReq = https.request(loginOpts, (res) => {
  ...
  https.get({
    ...
    res2.on('end', () => {
      ...
      console.log('Batch 16:', batch.map(c => '#' + c.id).join(', '));
        ...
        https.get({
          ...
          res3.on('end', () => {
            try {
              ...
              results[id] = {error: md.substring(0, 200)};
            }
            done++;
            if (done === chatIds.length) {
              ...
              chatIds.forEach(id => {
                ...
                const c = r.convo;
                out += '\n=== CHAT #' + id + ' === Phone: ' + (c.customer_phone||'?') + ' | State: ' + (c.current_state||c.state||'?') + ' | Msgs: ' + r.msgs.length + '\n';
                r.msgs.forEach(m => {
                  const dir = m.direction === 'outgoing' ? 'BOT' : (m.sender === 'human' ? 'ADMIN' : 'USER');
                  const content = (m.content || '').substring(0, 300);
                  out += dir + ': ' + content + '\n';
                });
              });
              fs.writeFileSync('tmp_batch16_chats.txt', out);
