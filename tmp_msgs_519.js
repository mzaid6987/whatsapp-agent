...
async function main() {
  ...
  const msgRes = await request({
    hostname: 'wa.nuvenza.shop', path: '/api/conversations/519/messages', method: 'GET',
  ...
  msgs.forEach(m => {
    ...
    if (debug && debug.collected) console.log('  collected:', JSON.stringify({name: debug.collected.name, city: debug.collected.city, address_parts: debug.collected.address_parts}));
    if (debug && debug.detected_intent) console.log('  intent:', debug.detected_intent);
  });
}
